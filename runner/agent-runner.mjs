...
// Where the controller finds us. See agent-socket.mjs for why it is a socket and not a port.
const SOCKET =
...
function main() {
	const turns = queue();
	// `handle` is a hoisted declaration, so the socket can be open before the line below it is read.
