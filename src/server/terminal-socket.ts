...
function open(connection: import("ws").WebSocket, session: Session): void {
	const ssh = spawn(
		...
		[
			...
			`export TERM=xterm-256color; stty rows ${session.rows} cols ${session.cols} 2>/dev/null; cd ${AGENT_CWD} 2>/dev/null; exec "\${SHELL:-/bin/sh}" -l`,
		],
	...
	connection.on("message", (data) => {
		ssh.stdin.write(
			typeof data === "string" ? data : Buffer.from(data as Buffer),
		);
	});
